...
import MainClientPage from "~app/(rooms)/MainClientPage";
...
const HomePage = async () => {
    ...
    // partnerId нужен для построения корректного wishlist peek query key.
    // fetchPartnerProfileServer обёрнут React.cache → повтор в prefetch ниже
    // бесплатный.
    ...
    await Promise.all([
        ...
        queryClient
            .prefetchQuery(
                createFetchEventsRangeServerQuery(nextPlanRange, today),
            )
            .catch(() => undefined),
        queryClient
